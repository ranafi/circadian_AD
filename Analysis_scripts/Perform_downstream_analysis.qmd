...
GWAS_cycling_AD = GWAS_cycling_AD_df%>% dplyr::select(Gene_Symbols) %>% unname %>% unlist

#Take union of those sets:
GWAS_genes_cycling_either = union(GWAS_cycling_CTL, GWAS_cycling_AD)
...
#perform diff_cycling analysis
pb <- progress_bar$new(total = length(GWAS_genes_cycling_either))
if(length(GWAS_genes_cycling_either) != 0){
GWAS_diff_rhythms_results = diff_rhyth(cyc_pred, tmm, GWAS_genes_cycling_either, useBatch = useBatch, pb = pb, percentile = percentile)
...
write_xlsx(list(GWAS_CTL_cycling = GWAS_cycling_CTL_df, GWAS_AD_cycling = GWAS_cycling_AD_df, GWAS_diff_rhyth = GWAS_diff_rhythms_results, GWAS_diff_mesor = diff_mesor_GWAS), path = paste0(path_to_cyclops_ordering, "/downstream_output/AD_GWAS_diff_rhythms_CyclingBHQ",BHQCutoff_str,".xlsx"))

GWAS_diff_rhythms_results
}
```
...
rnk_files = list.files(pattern = ".rnk")
#I dont want any remapping for these genes becase they are custom pathways
#that by construction are found in my rosmap humans.
