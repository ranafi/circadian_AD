...
####SHUFFLE COLUMNS CODE ########
# Random.seed!(1234)
# Random.seed!(42)
Random.seed!(1)
# Step 1: Identify columns where first and second rows are both "cond_0"
special_columns = filter(col -> TMM[1, col] == "cond_0" && TMM[2, col] == "cond_0", names(TMM)[2:end])

# Step 2: Choose one column from `special_columns` to place as the second column
chosen_column = rand(special_columns)

# Step 3: Shuffle the remaining columns (excluding RowNames and chosen_column)
remaining_columns = setdiff(names(TMM)[2:end], [chosen_column])
shuffled_columns = shuffle(remaining_columns)

# Step 4: Reorder DataFrame with RowNames fixed, chosen_column second, then shuffled columns
TMM = TMM[:, [names(TMM)[1], chosen_column, shuffled_columns...]]
