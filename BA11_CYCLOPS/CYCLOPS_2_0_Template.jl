...
base_path = joinpath(homedir(), "Box Sync", "Henry_stuff", "AD_project", "scROSMAP", "scrosmap_newAtlas", "scROSMAP_project", "BA11_CYCLOPS") # define the base path in which the data file, the seed gene list, and the sample collection time file are located
...
TMM = CSV.read(joinpath(data_path,  "BA11.csv"), DataFrame) # load data file in data folder

TMM = TMM[Not(1:6), :]
nth_seed_cutoff =  size(TMM, 1)-6                    #FIXME to "-1" with no covariates, "-2" with covariates.
nth_seed_cutoff = min(nth_seed_cutoff, 10000)
seed_genes = unique(CSV.read(joinpath(base_path, "seed_genes_ubiquity_cyclers.csv"), DataFrame, header = false)).Column1
...
eigendata, modeloutputs, correlations, bestmodel, parameters = CYCLOPS.Fit(TMM, seed_genes, training_parameters)
file_name, ~ = CYCLOPS.Align(TMM, modeloutputs, correlations, bestmodel, parameters, output_path)



smoothness_all = CYCLOPS.smoothness_metric(eigendata, bestmodel, modeloutputs; covariates = false)
touch(joinpath(output_path, "smoothness.txt"))
file = open(joinpath(output_path, "smoothness.txt"), "w")
